...
>   fmap f (Facet l auth unauth) = Facet l (fmap f auth) (fmap f unauth)
...
simple case, function apply to value

>   Raw f <*> Raw v = Raw $ (f v)

one function to multiple values; or (Raw f) <*> fv = fmap f fv

>   (Raw f) <*> (Facet label auth unauth) = fmap f (Facet label auth unauth)

multiple functions applied to 1 value

>   (Facet label authF unauthF) <*> Raw v = Facet label (authF <*> (Raw v)) (unauthF <*> (Raw v))

multiple functions apply to multiple values

>   (Facet leftTreeLabel leftTreeAuth leftTreeUnauth) <*> (Facet rightTreeLabel rightTreeAuth rightTreeUnauth) = Facet leftTreeLabel (Facet rightTreeLabel (leftTreeAuth <*> rightTreeAuth) (leftTreeAuth <*> rightTreeUnauth)) (Facet rightTreeLabel (leftTreeUnauth <*> rightTreeAuth) (leftTreeUnauth <*> rightTreeUnauth))
